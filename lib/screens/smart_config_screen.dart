...
class _SmartConfigScreenState extends State<SmartConfigScreen> {
  ...
  Future<void> _loadPreferences() async {
    ...
    final devices = await _prefs.getDevices(_currentServer!,_currentLogin!);
  ...
  Future<void> _sendConfigToAp() async {
    try {
      ...
      debugPrint('Sending config to AP mode');
      debugPrint('Target SSID: $targetSsid');
        ...
        try {
          ...
          if (responseData is Map && responseData.containsKey('mac') ) {
            ...
            if (deviceMacFromResponse != null &&
              ...
              debugPrint('MAC from device response: $deviceMac');
            } else {
              err = true;
            }
          }else {
          ...
          if (responseData is Map && responseData.containsKey('type') ) {
            ...
            if (deviceDatFromResponse != null &&
              ...
              debugPrint('DAT from device response: $deviceType');
            } else {
              err = true;
            }
          }else {
          ...
          if(err==true)
          {
            debugPrint('MAC адресс или тип датчика нераспознан!');
          ...
          debugPrint('Could not parse response: $e');
        }
  ...
  Future<void> _startSmartConfig() async {
    ...
    subscription = _provisioner.listen((result) {
      debugPrint('SmartConfig result: ${result.bssidText}');
    ...
    try {
      ...
      debugPrint('SmartConfig error: $e');
  ...
  Future<void> _sendConfigToDevice(String deviceIp, String deviceMacFromSmartConfig) async {
    try {
      ...
      debugPrint('Sending config to: http://$deviceIp/api/config');
      debugPrint('Config: $config');
